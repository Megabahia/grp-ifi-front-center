...
import { SolicitudesCreditos } from '../../../models/creditos';
...
export class SolicitudesCreditosComponent implements OnInit {
    ...
    toggleSidebar(name, id): void {
        if (id) {
            this._solictudesCreditosService.obtenersolicitudCredito(id).subscribe(
                (info) => {
                    const {
                        _id,
                        tomarSolicitud,
                    ...
                    this.actualizarCredito.fechaAprobacion =
    ...
    actualizarSolicitudCredito() {
        ...
        this.cargando = true;
        this._solictudesCreditosService
            .actualizarSolictudesCreditos(this.actualizarCreditoFormData)
            .subscribe(
                (info) => {
    ...
    async subirDocumentoAprobacion(event) {
        if (event.target.files && event.target.files[0]) {
            ...
            this.actualizarCreditoFormData.delete('documentoAprobacion');
            this.actualizarCreditoFormData.append(
                'documentoAprobacion',
                imagen,
